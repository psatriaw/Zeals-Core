    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        ...
        <div class="wrapper wrapper-content animated fadeInRight">
            <div class="row">
                ...
                <div class="ibox-title">
                    <h5>Stock Tersimpan pada Gudang</h5>
                </div>
                <div class="ibox-content">
                  @include('backend.flash_message')
                    ...
                    <table class="table table-striped table-bordered table-hover dataTables-example" id="datatable">
                      <thead>
                        <tr>
                            ...
                            <th>Waktu Pembelian/Penerimaan</th>
                            <th>Qty Pembelian</th>
                            <th>Harga Beli</th>
                            <th>Digunakan</th>
                            <th>Tersedia</th>
                          ...
                          if(@$items){
                            foreach ($items as $key => $value) {
                              ...
                              <tr>
                                ...
                                <td><?=date("Y-m-d H:i:s",$value->purchase_time)?></td>
                                <td class="text-right"><?=number_format($value->item_quantity,2,",",".")?></td>
                                <td class="text-right">Rp. <?=number_format($value->purchase_price,2,",",".")?></td>
                                <?php
                                  $total_quantity   = $total_quantity + $value->quantity;
                                  $available        = $value->item_quantity - $value->item_used;
                                  $total_used       = $total_used + $value->item_used;
                                  $total_available  = $total_available + $available;
                                ?>
                                <td class="text-right"><?=number_format(@$value->item_used,2,",",".")?></td>
                                <td class="text-right"><?=number_format(@$available,2,",",".")?></td>
                                <td>
                                  <?php if($previlege->isAllow($login->id_user,$login->id_department,$config['manage'])){?>
                                      <?php if($available<=0){?>
                                              <a class="btn btn-primary dim btn-xs disabled"> habis <i class="fa fa-times"></i></a>
                                      <?php }else{ ?>
                                              <a href="{{ url('/master/logistik/permintaan/manage/'.$transaksi->id_transaction.'/ambil-item/'.$data->id_material.'/pick/'.$value->id_purchase_detail) }}" class="btn btn-primary dim btn-xs confirm"> gunakan <i class="fa fa-angle-right"></i></a>
                                      <?php }?>
                                  <?php }?>
                                </td>
                            ...
                            ?>
                              <tr>
                                ...
                                <td class="text-right"><strong><?=number_format(@$total_used,0,",",".")?></strong></td>
                                <td class="text-right"><strong><?=number_format(@$total_available,0,",",".")?></strong></td>
                                <td class="text-right"></td>
                              </tr>
                            <?php
                          }else{
