    ...
    <div class="row wrapper border-bottom white-bg page-heading">
            <div class="col-lg-10">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="active">
                        <strong>Kelola Produk Reject</strong>
                    </li>
                </ol>
            </div>
            <div class="col-lg-2">
            ...
            <div class="row">
                <div class="col-lg-12">
                  <div class="ibox float-e-margins">
                    ...
                    {!! Form::model($data,['url' => url($main_url.'/storereject/'.$data->id_cart.'/'.$id_transaction), 'method' => 'post', 'id' => 'formmain', 'data-parsley-validate novalidate']) !!}
                              ...
                              if($produk){
                                foreach ($produk as $key => $value) {
                                  ...
                                  <tr>
                                    ...
                                    <td><div id="" class="label-area">0</div><div class="label-meter" id="slider_s_<?=$value->id_product?>"></div><div id="" class="label-area"><?=($value->quantity)?></div></td>
                                    <td>
                                      <input type="text" class="label-area" name="naik_<?=$value->id_product?>" id="naik_<?=$value->id_product?>">
                                    </td>
                                  </tr>
                                  <?php
                                }
                    ...
                    <div class="row">
                      <div class="form-group top15">
                          ...
                          <div class="col-sm-6 text-right">
                            <button class="btn btn-info" type="submit">Simpan Produk Reject</button>
                          </div>
                      </div>
                    </div>
                </div>
...
<script>
  ...
  if($produk){
    foreach ($produk as $key => $value) {
      ...
      $( "#slider_s_<?=$value->id_product?>" ).slider({
        value:0,
        min: 0,
        max: <?=($value->quantity)?>,
        step: 1,
        slide: function( event, ui ) {
          $( "#label-info-<?=$value->id_product?>" ).html(ui.value );
          $("#naik_<?=$value->id_product?>").val(ui.value);

        }
      });

      $("#naik_<?=$value->id_product?>").keyup(function(e){
        var nilai = $(this).val();
        $( "#slider_s_<?=$value->id_product?>").slider("value",nilai);
      });
      <?php
    }
  }
