    ...
    <div class="wrapper wrapper-content animated fadeInRight">
      <div class="row">
        <div class="col-lg-3 col-md-6">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <span class="label label-info pull-right">company</span>
                    <h5>Total Brands</h5>
                </div>
                <div class="ibox-content" id="sum-company">
        ...
        <div class="col-lg-3 col-md-6">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <span class="label label-info pull-right">campaigns</span>
                    <h5>Average Project</h5>
                </div>
                <div class="ibox-content" id="sum-average">
                  <i class="fa fa-spinner fa-spin fa-duotone"></i> loading
                </div>
            </div>
        </div>
            ...
            <div class="ibox-title">
              ...
              <?php if ($previlege->isAllow($login->id_user, $login->id_department, "penerbit-create")) { ?>
                <div class="ibox-tools">
                  <a class="btn btn-secondary btn-sm" href="{{ url('master/penerbit/create') }}">
                            ...
                            <div class="btn-group" role="group">
                                ...
                                <ul class="dropdown-menu">
                                  <?php if ($previlege->isAllow($login->id_user, $login->id_department, $config['edit'])) { ?>
                                    <li>
                                      <a href="{{ url('master/penerbit/edit/'.$value->id_penerbit) }}" class=""><i class="fa fa-paste"></i> edit</a>
                                    </li>
                                  <?php } ?>

                                  <?php if ($previlege->isAllow($login->id_user, $login->id_department, $config["view"])) { ?>
                                    <li>
                                      <a href="{{ url('master/penerbit/detail/'.$value->id_penerbit) }}" class=""><i class="fa fa-eye"></i> detail</a>
                                    </li>
                                  <?php } ?>

                                  <?php if ($previlege->isAllow($login->id_user, $login->id_department, $config["manage"])) { ?>
                                    <li>
                                      <a href="{{ url('master/penerbit/manage/'.$value->id_penerbit) }}" class=""><i class="fa fa-cogs"></i> manage</a>
                                    </li>
                                  <?php } ?>

                                  <?php if ($previlege->isAllow($login->id_user, $login->id_department, $config["remove"])) { ?>
                                    <li class="text-danger">
                                      <a data-id="{{ $value->id_penerbit }}" data-url="{{ url('master/penerbit/remove/' . $value->id_penerbit) }}" class="confirm"><i class="fa fa-trash"></i> remove</a>
                                    </li>
                                  <?php } ?>
                                </ul>
                              </div>
...
<script>
  var dataset = [
    "sum-company","sum-average"
  ];
  var datasetunit = [
    "Brands","Projects per Brand"
  ];
