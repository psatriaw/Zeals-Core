                              ...
                              if($data){
                                foreach ($data as $key => $value) {
                                  ...
                                  <tr>
                                    ...
                                    ?>
                                    <td><img src="<?=$path?>" class="img-thumbnail" style="width:64px;height:64px;"></td>
                                    <td><?=$value->permalink?></td>
                                    <td><?=$value->caption?> <?=($value->hashtag!="")?"<br>#".implode("<br>#",explode(",",$value->hashtag)):""?></td>
                                    ...
                                    <td>
                                      <?php if($previlege->isAllow($login->id_user,$login->id_department,$config['edit'])){?>
                                          <a href="{{ url($config['main_url'].'/edit/'.$value->id_post) }}" class="btn btn-info btn-outline dim btn-xs" title="edit"><i class="fa fa-paste"></i></a>
                                      <?php }?>

                                      <?php if($previlege->isAllow($login->id_user,$login->id_department,$config['view'])){?>
                                          <a href="{{ url($config['main_url'].'/detail/'.$value->id_post) }}" class="btn btn-info btn-outline dim btn-xs" title="detail"><i class="fa fa-eye"></i></a>
                                      <?php }?>

                                      <?php if($previlege->isAllow($login->id_user,$login->id_department,$config['remove'])){?>
                                          <a data-id="{{ $value->id_post }}" data-url="{{ url($config['main_url'].'/remove/') }}" class="btn btn-danger btn-outline dim btn-xs confirm" titl="remove"><i class="fa fa-trash"></i></a>
                                      <?php }?>
                                    </td>
                                  </tr>
                                  <?php
