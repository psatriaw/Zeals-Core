                        ...
                        <div class="ibox-title">
                            ...
                            <?php if ($previlege->isAllow($login->id_user, $login->id_department, $config['create'])) { ?>
                                <div class="ibox-tools">
                                    <a class="btn btn-secondary add-btn btn-sm tooltips" href="{{ url($main_url.'/create') }}" title="Tambah data">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <?php if ($previlege->isAllow($login->id_user, $login->id_department, $config['edit'])) { ?>
                                                            <a href="{{ url('master/banner/edit/'.$value->id_banner) }}" class="btn btn-primary btn-rounded dim btn-xs tooltips" title="ubah data <?= $value->title ?>" data-placement="top"><i class="fa fa-paste"></i> ubah</a>
                                                        <?php } ?>

                                                        <?php if ($previlege->isAllow($login->id_user, $login->id_department, $config['view'])) { ?>
                                                            <a href="{{ url('master/banner/detail/'.$value->id_banner) }}" class="btn btn-white btn-outline dim btn-xs tooltips" title="lihat detail data <?= $value->title ?>"><i class="fa fa-eye"></i> lihat</a>
                                                        <?php } ?>
                                                        <?php if ($previlege->isAllow($login->id_user, $login->id_department, $config['manage'])) { ?>
                                                            <a href="{{ url('master/banner/manage/'.$value->id_banner) }}" class="btn btn-white btn-outline dim btn-xs tooltips" title="kelola kecamatan <?= $value->title ?>"><i class="fa fa-cogs"></i> kelola</a>
                                                        <?php } ?>

                                                        <?php if ($previlege->isAllow($login->id_user, $login->id_department, $config['remove'])) { ?>
                                                            <a data-id="{{ $value->id_banner }}" data-url="{{ url('master/banner/remove/' . $value->id_banner) }}" class="btn btn-danger btn-outline dim btn-xs confirm tooltips" title="hapus data <?= $value->title ?>"><i class="fa fa-trash"></i> hapus</a>
                                                        <?php } ?>
                                                    </td>
                                                </tr>
                                        <?php
