                                  ...
                                  <ul class="dropdown-menu">
                                    <?php
                                      foreach ($statuses as $key => $value) {
                                          ?>
                                          <li><a href="<?=url($main_url.'/?page='.@$input['page'].'&keyword='.@$input['keyword'].'&short='.@$input['short'].'&shortmode='.@$input['shortmode'].'&status='.$key)?>"><?=$value?></a></li>
                                          <?php
                                      }
                                    ?>
                                  </ul>
                              ...
                              if($data){
                                foreach ($data as $key => $value) {
                                  ...
                                  <tr class="<?=@$bgcolors[$value->status]?>">
                                    ...
                                    <td><?=$value->purchase_title?></td>
                                    <td><?=$value->tipe_pembayaran?></td>
                                    <td><?=($value->receiver_name!="")?$value->receiver_name:"belum ada"?></td>
                                    <td>Rp. <?=number_format($value->estimation,2,",",".")?></td>
                                    ...
                                    <td>
                                      <?php if($previlege->isAllow($login->id_user,$login->id_department,$methods['edit'])){?>
                                          <a href="{{ url($main_url.'/edit/'.$value->id_purchase) }}" class="btn btn-primary dim btn-xs"><i class="fa fa-paste"></i> ubah</a>
                                      <?php }?>

                                      <?php if($previlege->isAllow($login->id_user,$login->id_department,$methods['view'])){?>
                                          <a href="{{ url($main_url.'/cart/'.$value->id_purchase) }}" class="btn btn-white btn-outline dim btn-xs"><i class="fa fa-cogs"></i> kelola</a>
                                      <?php }?>

                                      <?php if($value->status!="received"){ ?>
                                        <?php if($previlege->isAllow($login->id_user,$login->id_department,$methods['remove'])){?>
                                            <a data-id="{{ $value->id_purchase }}" data-url="{{ url($main_url.'/remove/') }}" class="btn btn-danger btn-outline dim btn-xs confirm"><i class="fa fa-trash"></i> hapus</a>
                                        <?php }?>

                                        <?php if($previlege->isAllow($login->id_user,$login->id_department,$methods['receive'])){?>
                                            <a data-id="{{ $value->id_purchase }}" data-url="{{ url($main_url.'/terima/') }}" class="btn btn-success btn-outline dim btn-xs confirm"><i class="fa fa-check"></i> terima</a>
                                        <?php }?>
                                      <?php } ?>
                                    </td>
                                  </tr>
