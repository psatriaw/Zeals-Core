                            ...
                            <?php
                              ...
                              if($detail_pesanan){
                                foreach ($detail_pesanan as $key => $value) {
                                  $counter++;
                                  if($premodule==0){
                                    $premodule = $value->store_kode_pesanan_id;
                                    // dd($premodule);
                                    ?>
                                    <tr> 
                                      <th colspan="9" class="bg-primary"><strong><?=strtoupper($value->store_kode_pesanan_id)?></strong></th>
                                    </tr>
                                    <?php
                                    $counter = 1;
                                  }elseif($value->store_kode_pesanan_id!=$premodule){
                                    $premodule = $value->store_kode_pesanan_id;
                                    ?>
                                    <tr>
                                      <th colspan="9" class="bg-primary"><strong><?=strtoupper($value->store_kode_pesanan_id)?></strong></th>
                                  ...
                                  <tr>
                                        ...
                                        <td><?=$value->jenis_ambil?></td>
                                        <td><?=$value->tanggal_ambil?></td>
                                        <td><?=$value->alamat_antar?></td>
                                        <td> <strong> <?=$value->status?> </strong></td>
                                        <td>
                                                <a class="btn btn-primary btn-xs" href="{{ route('detail_pesanan', $value->id) }}"><i class="fa fa-eye"></i> detail</a>
                                                    <!-- <button class="btn btn-white btn-outline btn-xs" href="{{ route('pelunasan.kelola', $value->id) }}"><i class="fa fa-cogs"></i>kelola</button> -->

                                                <?php if($value->status == "Diproses") { ?>
                                                    {!! Form::model($value,['url' => url($main_url.'/update/'.$value->id), 'method' => 'post', 'id' => 'formmain','class' => 'form-horizontal', 'data-parsley-validate novalidate']) !!}
                                                ...
                                                <?php if($value->status == "Lunas"|| $value->status == "Diterima") { ?>
                                                    {!! Form::model($value,['url' => url($main_url.'/update/'.$value->id), 'method' => 'post', 'id' => 'formmain','class' => 'form-horizontal', 'data-parsley-validate novalidate']) !!}
