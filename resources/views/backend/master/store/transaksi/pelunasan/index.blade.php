                ...
                <table id="transaksi" class="table table-bordered table-hover">
                    ...
                    @foreach($kode_pesanan as $d)
                        <tr>
                            ...
                            <a href="{{ route('pelunasan_edit_admin',$d->store_kode_pesanan_id) }}" class="btn btn-primary dim btn-xs"><i class="fa fa-paste"></i> ubah</a>
                            <?php }?>
                            </td>
                            <td> 
                            <div class="form-group">
                                <div class="col-sm-6 text-right">
                                    <a class="btn btn-primary btn-xs" href="{{ route('detail_pesanan', $d->store_kode_pesanan_id) }}"><i class="fa fa-eye"></i> detail</a>
                                        <!-- <button class="btn btn-white btn-outline btn-xs" href="{{ route('pelunasan.kelola', $d->store_kode_pesanan_id) }}"><i class="fa fa-cogs"></i>kelola</button> -->
 
                                    <?php if($d->status == "Diproses") { ?>
                                        {!! Form::model($d,['url' => url($main_url.'/update/'.$d->store_kode_pesanan_id), 'method' => 'post', 'id' => 'formmain','class' => 'form-horizontal', 'data-parsley-validate novalidate']) !!}
                                        <input type="hidden" name="status" value="{{ $d->status }}">
                                        <input type="hidden" name="id" value="{{ $d->store_kode_pesanan_id }}">
                                    ...
                                    <?php if($d->status == "Diterima") { ?>
                                        {!! Form::model($d,['url' => url($main_url.'/update/'.$d->store_kode_pesanan_id), 'method' => 'post', 'id' => 'formmain','class' => 'form-horizontal', 'data-parsley-validate novalidate']) !!}
                                        <input type="hidden" name="status" value="Diambil Lunas">
                                        <input type="hidden" name="id" value="{{ $d->store_kode_pesanan_id }}">
                                        <button class="btn btn-info btn-outline btn-xs" type="submit"><i class="fa fa-money"></i> Diambil Lunas</button >
                                        {!! Form::close() !!}
                                    <?php } ?>
                                    ...
                                    <?php if($d->status == "Diterima") { ?>
                                        ...
                                        <input type="hidden" name="id" value="{{ $d->store_kode_pesanan_id }}">
                                        <button class="btn btn-info btn-outline btn-xs" type="submit"><i class="fa fa-money"></i> Diambil Dulu</button >
                                        <!-- <button class="btn btn-primary btn-xs"><i class="fa fa-paste"></i> ubah</button> -->
                                        {!! Form::close() !!}
                                    <?php } ?>
        ...
        <div class="ibox-content">
            <div class="table-responsive">
                ...
                <table id="transaksi" class="table table-bordered table-hover">
                    ...
                    <tbody>
                    @php $i=1 @endphp
                    @foreach($kode_pesanan_luar as $d)
                        <tr>
                            <td>{{ $i++ }}</td>
                            <td>{{ $d->store_kode_pesanan_id}}</td>
                            <td>{{ $d->tempat_pesan }}</td>
                            <!-- <td>{{ $d->qty }}</td> -->
                            <td>{{ $d->nama_pemesan}}</td>
                            <td>{{ $d->jenis_ambil}}</td>
                            <td>{{ $d->tanggal_ambil}}</td>
                            ...
                            <div class="form-group">
                                <div class="col-sm-6 text-right">
                                    ...
                                    <?php if($d->status == "Diterima" && $d->dp == 0) { ?>
                                        {!! Form::model($d,['url' => url($main_url.'/update/'.$d->store_kode_pesanan_id), 'method' => 'post', 'id' => 'formmain','class' => 'form-horizontal', 'data-parsley-validate novalidate']) !!}
                                        <input type="hidden" name="status" value="{{ $d->status }}">
                                        <input type="hidden" name="id" value="{{ $d->store_kode_pesanan_id }}">
