    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        ...
        <div class="wrapper wrapper-content animated fadeInRight">
            ...
            <div class="table-responsive">
                        ...
                        <tr>
                            <td>{{ $k->tanggal }}</td>
                            <td>{{ $k->gograb_kategori }}</td>
                            <td>Rp. {{ number_format($k->jumlah) }}</td>
                            <td>
                                      <?php if($previlege->isAllow($login->id_user,$login->id_department,$methods['edit'])){?>
                                          <a href="{{ route('gograb.edit',$k->id) }}" class="btn btn-primary dim btn-xs"><i class="fa fa-paste"></i> ubah</a>
                                      <?php }?>

                                      <?php if($previlege->isAllow($login->id_user,$login->id_department,$methods['remove'])){?>
                                          <a data-id="{{ $k->id }}" data-url="{{ route('gograb.destroy',$k->id) }}" class="btn btn-danger btn-outline dim btn-xs confirm"><i class="fa fa-trash"></i> hapus</a>
                                      <?php }?>
                                    </td>
                        </tr>
                        @endforeach
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">Tambah Gograb</h4>
