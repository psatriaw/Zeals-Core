...
<div id="wrapper">
  ...
  <div id="page-wrapper" class="gray-bg">
    ...
    <div class="row wrapper border-bottom white-bg page-heading">
            <div class="col-lg-10">
                <h2>Laporan Harian Total Penjualan Sf</h2>
                <ol class="breadcrumb">
                    <li>
        ...
        <div class="ibox">
            <div class="ibox">
                <div class="ibox-title">
                    ...
                    Tambah Sf <p>(Peringatan Data yang sudah dibuat tidak bisa dihapus)</p>
                </div>
                ...
                <div class="form-group"><label class="col-sm-2 control-label">Sales Force</label>
                    <div class="col-sm-6 col-xs-12"> 
                    <label class="col-sm-2 control-label">{{Session::get("toko")->nama}}</label>
                    <input type="hidden" name="sf_id" id="sf_id" value="{{Session::get("toko")->nama}}" class="form-control" autofocus required>
                    </div>
                </div>
                <div class="form-group {{ ($errors->has('diskon')?"has-error":"") }}"><label class="col-sm-2 control-label">Nama Toko</label>
                    <div class="col-sm-6 col-xs-12">        
                ...
                <div class="form-group {{ ($errors->has('total')?"has-error":"") }}"><label class="col-sm-2 control-label">Nominal Omset</label>
                    <div class="col-sm-6 col-xs-12">        
                ...
                <div class="form-group {{ ($errors->has('diskon')?"has-error":"") }}"><label class="col-sm-2 control-label">Diskon</label>
                    <div class="col-sm-6 col-xs-12">        
                        <input type="number" name="diskon" class="form-control" autofocus required>
                    </div>
                </div>
                    <!--<select name="sf_id" id="sf_id" class="form-control">
                    <option value="">- Pilih -</option>
                    @foreach($optsf as $d)
                    <option value="{{$d->id}}">{{$d->nama}}</option>
                    @endforeach-->
            ...
            <div class="table-responsive">
                        ...
                        <tr>
                            ...
                            <td>Rp. {{ number_format($k->diskon) }}</td>
                            <td>
                                      <?php if($previlege->isAllow($login->id_user,$login->id_department,$methods['edit'])){?>
                                          <a href="{{ route('sf.edit',$k->id) }}" class="btn btn-primary dim btn-xs"><i class="fa fa-paste"></i> ubah</a>
                                      <?php }?>

                                      <?php if($previlege->isAllow($login->id_user,$login->id_department,$methods['remove'])){?>
                                          <a data-id="{{ $k->id }}" data-url="{{ route('sf.destroy',$k->id) }}" class="btn btn-danger btn-outline dim btn-xs confirm"><i class="fa fa-trash"></i> hapus</a>
                                      <?php }?>
                                    </td>
                        </tr>
                        @endforeach
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">Tambah Sf</h4>
