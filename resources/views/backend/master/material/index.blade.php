                              ...
                              if($data){
                                foreach ($data as $key => $value) {
                                  ...
                                  <tr class="<?=@$bgcolors[$value->status]?> <?=($value->stock<=$value->minimum_stock && ($previlege->isAllow($login->id_user,$login->id_department,$methods['edit'])))?"danger":""?>">
                                    ...
                                    <?php if($value->is_package==""){ ?>
                                        <td><?=number_format($value->stock,2,",",".")?></td>
                                    <?php }else{ ?>
                                        <td>mengikuti paket</td>
                                    <?php }?>
                                    <td><?=$value->material_unit?></td>
                                    <?php if(($previlege->isAllow($login->id_user,$login->id_department,$methods['edit']))){ ?>
                                    <td><?=number_format($value->minimum_stock,2,",",".")?></td>
                                    <?php } ?>
                                    <td><?=$value->type_material?></td>
                                    ...
                                    <td>
                                      <?php if($previlege->isAllow($login->id_user,$login->id_department,$methods['edit'])){?>
                                          <a href="{{ url($main_url.'/edit/'.$value->id_material) }}" class="btn btn-primary dim btn-xs"><i class="fa fa-paste"></i> ubah</a>
                                      <?php }?>

                                      <?php if($previlege->isAllow($login->id_user,$login->id_department,$methods['view'])){?>
                                          <a href="{{ url($main_url.'/detail/'.$value->id_material) }}" class="btn btn-white btn-outline dim btn-xs"><i class="fa fa-eye"></i> detail</a>
                                      <?php }?>
                                      <?php if($value->is_package==""){ ?>
                                          <?php if($previlege->isAllow($login->id_user,$login->id_department,$methods['manage'])){ ?>
                                              <a data-id="{{ $value->id_material }}" data-unit="{{ $value->material_unit }}"  class="btn btn-white btn-outline dim btn-xs purchase"><i class="fa fa-shopping-basket"></i> beli</a>
                                          <?php }?>
                                      <?php }else{ ?>
                                          <?php if($previlege->isAllow($login->id_user,$login->id_department,$methods['manage'])){ ?>
                                              <a class="btn btn-white btn-outline dim btn-xs disabled" disabled="true"><i class="fa fa-shopping-basket"></i> beli</a>
                                          <?php }?>
                                      <?php }?>

                                      <?php if($previlege->isAllow($login->id_user,$login->id_department,$methods['remove'])){?>
                                          <a data-id="{{ $value->id_material }}" data-url="{{ url($main_url.'/remove/') }}" class="btn btn-danger btn-outline dim btn-xs confirm"><i class="fa fa-trash"></i> hapus</a>
                                      <?php }?>
                                    </td>
                                  </tr>
                                  <?php
