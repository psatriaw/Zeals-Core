                                    ...
                                    <?php
                                      foreach($shorter as $key=>$val){
                                        ...
                                        <li class="<?=($key==Request::input("short"))?"active":""?>">
                                            <a href="<?=url($main_url.'?page='.@$input['page'].'&keyword='.@$input['keyword'].'&short='.$key.'&shortmode='.@$input['shortmode'])?>"><?=$val?></a>
                                        </li>
                                        <?php
                                      }
                                    ?>
                              ...
                              <div class="input-group-btn bg-white">
                                  ...
                                  <ul class="dropdown-menu">
                                      <li class="<?=("asc"==Request::input("shortmode"))?"active":""?>">
                                        <a href="<?=url($main_url.'?page='.@$input['page'].'&keyword='.@$input['keyword'].'&short='.@$input['short'].'&shortmode=asc')?>">asc</a>
                                      </li>
                                      <li class="<?=("desc"==Request::input("shortmode"))?"active":""?>">
                                        <a href="<?=url($main_url.'?page='.@$input['page'].'&keyword='.@$input['keyword'].'&short='.@$input['short'].'&shortmode=desc')?>">desc</a>
                                      </li>
                                  </ul>
                              </div>
                              ...
                              if($data){
                                foreach ($data as $key => $value) {
                                  ...
                                  <tr>
                                    ...
                                    <td><img src="<?=$value->brand_avatar?>" class="img img-thumbnail img-circle" style="width:80px;"></td>
                                    <td><?=$value->brand_permalink?></td>
                                    ...
                                    <td>
                                      <?php if($previlege->isAllow($login->id_user,$login->id_department,$config['edit'])){?>
                                          <a href="{{ url($config['main_url'].'/edit/'.$value->id_brand) }}" class="btn btn-info btn-outline dim btn-xs" title="edit"><i class="fa fa-paste"></i></a>
                                      <?php }?>

                                      <?php if($previlege->isAllow($login->id_user,$login->id_department,$config['view'])){?>
                                          <a href="{{ url($config['main_url'].'/detail/'.$value->id_brand) }}" class="btn btn-info btn-outline dim btn-xs" title="detail"><i class="fa fa-eye"></i></a>
                                      <?php }?>

                                      <?php if($previlege->isAllow($login->id_user,$login->id_department,$config['remove'])){?>
                                          <a data-id="{{ $value->id_brand }}" data-url="{{ url($config['main_url'].'/remove/') }}" class="btn btn-danger btn-outline dim btn-xs confirm" titl="remove"><i class="fa fa-trash"></i></a>
                                      <?php }?>
                                    </td>
                                  </tr>
                                  <?php
