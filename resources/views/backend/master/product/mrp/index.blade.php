                ...
                <div class="col-lg-12">
                  <div class="ibox float-e-margins">
                      <div class="ibox-title">
                          <h5>Tabel Data Produk dalam Produk "<?=$data->product_name?>"</h5>
                      ...
                      <div class="ibox-title">
                          ...
                          <?php if($previlege->isAllow($login->id_user,$login->id_department,$config['create']) && $previlege->isAllow($login->id_user,$login->id_department,$config['mrp'])){?>
                          <div class="ibox-tools">
                              <a class="btn btn-primary btn-sm" href="{{ url($main_url.'/mrp/'.$data->id_product.'/create-product') }}">
                          ...
                          <table class="table table-striped table-bordered table-hover dataTables-example" id="datatable">
                            ...
                            <tbody>
                              <?php
                                ...
                                if(@$product_on_product){
                                  foreach ($product_on_product as $key => $value) {
                                    $counter++;
                                    ?>
                                    <tr>
                                      <td><?=$counter?></td>
                                      ...
                                      <td>
                                        <?php if($previlege->isAllow($login->id_user,$login->id_department,$config['edit']) && $previlege->isAllow($login->id_user,$login->id_department,$config['mrp'])){?>
                                            <a href="{{ url($main_url.'/mrp/'.$data->id_product.'/edit-product/'.$value->id_product_on_product) }}" class="btn btn-primary dim btn-xs"><i class="fa fa-paste"></i> ubah</a>
                                        <?php }?>

                                        <?php if($previlege->isAllow($login->id_user,$login->id_department,$config['remove']) && $previlege->isAllow($login->id_user,$login->id_department,$config['mrp'])){?>
                                            <a data-id="{{ $value->id_product_on_product }}" parent-id="{{ $data->id_product }}" data-url="{{ url($main_url.'/mrp/remove-product/') }}" class="btn btn-danger btn-outline dim btn-xs confirm"><i class="fa fa-trash"></i> hapus</a>
                                        <?php }?>
                                      </td>
                                    </tr>
                                    <?php
