      ...
      <div class="row justify-content-md-center">
        <div class="col-md-4 col-12">
          <ul class="reg nav nav-tabs justify-content-center">
            <li class="nav-item">
              <a class="nav-link" aria-current="page" href="{{ url('register') }}">Pembeli</a>
            </li>
            <li class="nav-item">
              <a class="nav-link active" href="{{ url('register/mitra') }}">Mitra</a>
            </li>
          </ul>
        </div>
      </div>
        ...
        <div class="row justify-content-md-center">
          <div class="col-12 col-sm-6 col-md-4">
            ...
            <div class="form-group checkinput {{ ($errors->has('first_name')?"has-error":"") }}">
                ...
                <div id='err_first_name' class='areawarn'></div>
            </div>
            <div class="form-group checkinput {{ ($errors->has('email')?"has-error":"") }}">
                ...
                <div id='err_email' class='areawarn'></div>
            </div>
            ...
            <div class="form-group checkinput {{ ($errors->has('phone')?"has-error":"") }}">
                ...
                <div id='err_phone' class='areawarn'></div>
            </div>
            ...
            <div class="form-group checkinput {{ ($errors->has('password')?"has-error":"") }}">
                ...
                <div id='err_password' class='areawarn'></div>
            </div>
                ...
                <div id='err_confirm_password' class='areawarn'></div>
            </div>
            <br>
            <label>Informasi Warung Anda</label>
            <div class="form-group checkinput {{ ($errors->has('shop_name')?"has-error":"") }}">
                {!! Form::text('shop_name', null, ['class' => 'form-control','placeholder' => 'Nama Warung Anda','id'=> "shop_name"]) !!}
                {!! $errors->first('shop_name', '<p class="bg-danger p-xs b-r-sm m-t">:message</p>') !!}
                <div id='err_shop_name' class='areawarn'></div>
            </div>
            <div class="form-group checkinput {{ ($errors->has('ktp')?"has-error":"") }}">
                {!! Form::text('ktp', null, ['class' => 'form-control','placeholder' => 'No KTP','id'=> "ktp"]) !!}
                {!! $errors->first('ktp', '<p class="bg-danger p-xs b-r-sm m-t">:message</p>') !!}
                <div id='err_ktp' class='areawarn'></div>
            </div>
            <div class="form-group checkinput {{ ($errors->has('nama_ktp')?"has-error":"") }}">
                {!! Form::text('nama_ktp', null, ['class' => 'form-control','placeholder' => 'Nama Sesuai KTP','id'=> "nama_ktp"]) !!}
                {!! $errors->first('nama_ktp', '<p class="bg-danger p-xs b-r-sm m-t">:message</p>') !!}
                <div id='err_nama_ktp' class='areawarn'></div>
            </div>
            <div class="form-group checkinput {{ ($errors->has('alamat_warung')?"has-error":"") }}">
                {!! Form::textarea('alamat_warung', null, ['class' => 'form-control checkinput','placeholder' => 'Alamat Warung', 'rows' => 3,'id'=>'alamat_warung']) !!}
                {!! $errors->first('alamat_warung', '<p class="bg-danger p-xs b-r-sm m-t">:message</p>') !!}
            </div>


            <br>
            <label>Informasi Pengiriman Stock</label>
    ...
    $("#formmain").submit(function(e){
      ...
      .done(function(result){
        ...
        if(result.status=="success"){
          ...
          for(var i in errors){
			      console.log(i);
            $("#err_"+i).html("<div class='text-danger'>"+errors[i][0]+"</div>");
