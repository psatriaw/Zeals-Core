...
<?php 
    ...
    $title  = "";
    $subtitle = "";
	$wilayah = json_decode(file_get_contents(url('/api/v1/wilayah')));
      ...
      <div class="row">
        ...
        <div class="col-xs-12 col-md-5 reg-right">
          ...
          <form class="m-t top30 text-left" role="form" action="{{ $submit_url }}" method="POST" id="registerForm">
			  ...
			  <div class="form-group">
                  <label>City/Domicile</label>
                  <div class="input-group">
					  <select name="id_wilayah" class="form-control">
						<?php if($wilayah){ ?>
							<?php foreach($wilayah as $index=>$wil){ ?>
							  <option value="<?=$wil->id_wilayah?>"><?=ucfirst($wil->namakab)?></option>
							<?php } ?>
						<?php } ?>
					  </select>
                  </div>
