...
class WishlistController extends Controller{
  ...
  public function __construct(){
    ...
    $config['main_url'] = "master/wishlist";
  ...
  public function index(Request $request){
    ...
    if($login){
      if($this->previlege_model->isAllow($login->id_user,$login->id_department,$this->config['view'])){
        ...
        $data       = $this->wishlist_model->getData($str, $limit, $keyword, $short, $shortmode);

        $totaldata  = $this->wishlist_model->countData($keyword);
        ...
        $view     = View::make("backend.wishlist.index",$datacontent);
        $content  = $view->render();
  ...
  public function remove(Request $request){
    ...
    if($login){
      if($this->previlege_model->isAllow($login->id_user,$login->id_department,$this->config['remove'])){
        $id = $request->input("id"); 
        $remove = $this->wishlist_model->removeData($id);
