        ...
        <div class="col-lg-12">
          <div class="ibox float-e-margins">
            <div class="ibox-title">
              <h5>Tabel Data Deviden</h5>
              <?php if ($previlege->isAllow($login->id_user, $login->id_department, "deviden-view")) { ?>
                <div class="ibox-tools">
                  <a class="btn btn-secondary btn-sm" href="<?php echo e(url($main_url.'/create/'.@$id_penerbit.'?backlink='.$main_url)); ?>">
                ...
                <table class="table table-striped table-bordered table-hover dataTables-example" id="datatable">
                  ...
                  <tbody>
                    ...
                    ?>
                        <tr>
                          <td><?= $counter ?></td>
                          <td><strong><?= $value->invoice_code ?></strong></td>
                          <td><?= $value->campaign_title ?></td>
                          <td><?= $value->rups_code ?></td>
                          <td><?= $value->nama_penerbit ?></td>
                          <td class="text-right">Rp. <?= number_format($value->keuntungan) ?></td>
                          <td class="text-right">Rp. <?= number_format($value->fee_total) ?></td>
                          <td class="text-right">Rp. <?= number_format($value->pajak_total) ?></td>
                          <td class="text-right">Rp. <?= number_format($value->total_tagihan) ?></td>
                          <td><?= date("Y-m-d H:i:s", $value->time_created) ?></td>
                          <td>
                            <?php if ($previlege->isAllow($login->id_user, $login->id_department, $config['edit'])) { ?>
                              <a href="<?php echo e(url('master/deviden/edit/'.$value->id_deviden)); ?>" class="btn btn-primary dim btn-xs btn-rounded"><i class="fa fa-paste"></i> ubah</a>
                            <?php } ?>

                            <?php if ($previlege->isAllow($login->id_user, $login->id_department, $config['view'])) { ?>
                              <a href="<?php echo e(url('master/deviden/detail/'.$value->id_deviden)); ?>" class="btn btn-white btn-outline dim btn-xs"><i class="fa fa-eye"></i> kelola</a>
                            <?php } ?>

                            <?php if ($previlege->isAllow($login->id_user, $login->id_department, $config['remove'])) { ?>
                              <a data-id="<?php echo e($value->id_deviden); ?>" data-url="<?php echo e(url('master/deviden/remove/' . $value->id_penerbit)); ?>" class="btn btn-danger btn-outline dim btn-xs confirm"><i class="fa fa-trash"></i> hapus</a>
                            <?php } ?>
                          </td>
                        </tr>
                    <?php
...
<?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/myzeals/resources/views/backend/master/deviden/index.blade.php ENDPATH**/ ?>